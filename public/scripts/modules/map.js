...
export async function nearbySearch(center,radius,type) {
  ...
  const request = {
    // required parameters
    fields: ["displayName", "location", "businessStatus", "photos", "rating"],
    locationRestriction: {
      center: center,
      radius: radius,
    },
...
export function updatePlaceNumber(types){
  ...
  if (numPlaces===0 || numPlaces===1)
    document.getElementById('place-num').innerText=`${totalNum} Place`;
  else
    document.getElementById('place-num').innerText=`${totalNum} Places`;
}
